...
ReinAI公式コミュニティサーバー用のbotです。主な機能:

1. **`/verify`** — メンバーがReinAIアカウントと連携すると、指定したロールを自動付与します。
2. **`/verify-reinchat`** — 上記とは別に、REINChatアカウントと直接連携できます(ReinAIアカウントの有無に関係なく利用可能)。どちらか一方でも連携すれば認証済みロールが付与されます。`REINCHAT_URL`が未設定の場合はこの機能自体が無効になります(既存デプロイに影響しません)。
3. **お知らせの自動転送** — reinai-admin(管理ダッシュボード)の「お知らせ」ページ、またはREINChat管理パネルの「お知らせ」に投稿した内容は、ReinAI/REINChat双方に相互転送された上で、このbotが指定したDiscordチャンネルへ自動投稿します。
4. **入退室ログ・メッセージログ・ボイスログ** — 下記「新機能」参照。
5. **管理者コマンド(65個)** — 「Rein管理者」ロールを持つ人だけが実行できる、ユーザー管理・モデレーション・コンテンツ運営・セキュリティ・ユーティリティ・ロール/招待・ボイス・メモ・リマインダー系のコマンド群。下記「管理者コマンド一覧」参照。`/user-info`はReinAI連携・REINChat連携の両方の状況を表示します。
...
`.env.example` の全項目を埋めてください。`DISCORD_BOT_SECRET` は、reinai-appのVercel環境変数 `DISCORD_BOT_SECRET` と**同じ値**にする必要があります(このbotとreinai-appの間の認証に使う共有シークレットです)。`/verify-reinchat`を使う場合は`REINCHAT_URL`も設定し、REINChat側のVercel環境変数`DISCORD_BOT_SECRET`にも同じ値を設定してください(未設定でもbot自体は問題なく動作し、この機能だけが無効になります)。
